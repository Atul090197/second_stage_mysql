...
exports.signUp = async (req, res) => {
  try {
    const { buyer_name, email, phone_number, user_name, password,
      license_state, license_number, guest_token, seller_sigup
      , seller_card_number, buyer_card_number } = req.body;
      ...
      if (data.length !== 0) {
        ...
        transporter.sendMail(mailOptions, async function (error, info) {
          if (error) {
            ...
            const update_guest_user_cart = await update_guest_cart(guest_token, new_user_id);
            const update_guest_user_whislist = await update_guest_whislist(guest_token, new_user_id);
